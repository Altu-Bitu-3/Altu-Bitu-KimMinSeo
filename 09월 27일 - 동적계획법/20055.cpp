...
vector<info> belt;  // 컨테이너 벨트 정보 (info : 내구도, 로봇 존재 여부)
int zero_power;     // 내구도가 0인 벨트 칸의 개수


int minusPosition(int n, int pos) { // 인덱스 감소시키는 함수
    if (--pos >= 0) {   // 벨트 위쪽인 경우
        return pos;
    }
    return n * 2 - 1;   // 벨트 아래쪽인 경우
}

void second(int n, int start, int end) {
    int cur = end;

    // end - 1 부터 start 까지 검사
    while (cur != start) { 
        ...
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구성 1이상인 경우 -> 로봇 옮김
            belt[cur].is_on = false;  // 로봇 존재 true
            belt[next].power--;       // 내구도 1 감소
            
            if (next != end) {           // 내리는 위치 아닐 경우만
                belt[next].is_on = true; // 로봇 옮기기
            }
            
            if (belt[next].power == 0) {  // 내구도 0되면 
...
int main() {
    ios_base::sync_with_stdio(false);   // 시간 초과 예방
    cin.tie(NULL);

    int n, k;  // n : 컨베이너 벨트 길이, k : 종료 기준 칸 수

    cin >> n >> k; // 입력
    belt.assign(n * 2, { 0, false });  // {내구도 0, 로봇 존재 false}로 초기화

    for (int i = 0; i < n * 2; i++) {
        cin >> belt[i].power;          // 맨 처음 내구도 현황 초기화
    }
    ...
    while (true) {
        ...
        if (zero_power >= k) {  // 내구도가 0인 칸의 개수가 K개 이상이라면 종료
            break;
        }
    }
